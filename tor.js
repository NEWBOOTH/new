...
app.get('/tor/:movieId', async (req, res) => {
    ...
    try {
      ...
      if (torrents && torrents.length > 0) {
        ...
        const sanitizedTorrent = {
          ...bestTorrent,
          quality: escapeHtml(bestTorrent.quality),
          type: escapeHtml(bestTorrent.type),
          size: formatBytes(bestTorrent.size), //Use a helper function for better readability
          hash: escapeHtml(bestTorrent.hash),
          url: bestTorrent.url //URLs generally don't need escaping, but you could validate it here
        };


        res.render('resTor', { torrent: sanitizedTorrent, movieTitle: escapeHtml(movie.title) }); 
      } else {
        res.status(404).send('Torrents not found for this movie.');
      }
    } catch (error) {
      console.error('Error fetching torrent data:', error);
      if (error.status) {
        res.status(error.status).send(error.message); //Send specific error if available
      } else {
        res.status(500).send('Error fetching torrent data.');
      }
    }
  });
//Helper function to escape HTML (replace with your templating engine's equivalent if available)
function escapeHtml(unsafe) {
  return unsafe
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
